      ...
      <div id="top-navbar-collapse" :class="['collapse', 'navbar-collapse', { in: showCollapsedNav }]">
        <ul class="nav navbar-nav">
          <li
            ...
            :key="item.id"
          >
            <router-link v-if="index === 0" :to="'/'" @click.native="changeNavIndex(index)">{{
              item.title
            }}</router-link>
            <router-link v-else-if="index === 5" :to="'/category/special'" @click.native="changeNavIndex(index)">{{
              item.title
            }}</router-link>
            <router-link v-else :to="'/category/' + item.id" @click.native="changeNavIndex(index)">{{
              item.title
            }}</router-link>
          </li>
        </ul>
